...
class API_data_generator():
    ...
    def __init__(self, API_list, term_list, cache_path, output_path):
        """
        Parameters:
    ...
    def build_search_terms(self):
        ...
        for i in range(len(self.term_list)):
            if i != len(self.term_list)-1:
                combined_keywords += self.term_list[i] + ' OR '
            else:
                combined_keywords += self.term_list[i] + ' '

        return combined_keywords

    def build_query_dict(self, issn_list, year_list):
        """
    ...
    def get_piis(self, year_list, config_path='/Users/nisarg/.scopus/config.ini', keymaster=False):
        """
        ...
        for i in range(len(issn_list)):
            ...
            os.mkdir(f'{self.output_path}{journal_list[i]}')
            with open(f'{self.output_path}{journal_list[i]}/{journal_list[i]}.json','w') as file:
                json.dump(issn_dict, file)

            with open(f'{self.output_path}{journal_list[i]}/{journal_list[i]}.txt','w') as file2:
    ...
    def make_dataframe(self, dataframe_path):
        ...
        for directory in directory_list:
            json_dict = load_journal_json(f'{self.output_path}/{directory}/{directory}.json')
        ...
        df.to_pickle(dataframe_path +'.pkl')
    ...
    def get_fulltexts(self, fulltext_output_path):
        """
        ...
        for directory in directory_list:
            ...
            json_file = f'{self.output_path}/{directory}/{directory}.json'
